...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to seed AI settings", details: String(error) },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "AI Settings seeded successfully (DEV mode)",
      url: "/api/seed-ai-settings (POST with admin auth required in production)"
    });
